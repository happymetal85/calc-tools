...
HT.register({
  id: 'tarot', cat: '유용한도구', order: 0.45, original: false, name: '타로 한 장 뽑기', keywords: '타로 카드 한 장 오늘의 타로 메이저 아르카나 질문 정방향 역방향',
  ...
  render(root) {
    ...
    function pick() {
      ...
      const share = `[타로] "${question}" → ${c[1]} (${c[2]}) ${rev ? '역방향' : '정방향'}: ${rev ? c[4] : c[3]} — 한손도구 타로 한 장`;
      result.innerHTML = ''; result.append(HT.el('div', { style: 'display:grid;grid-template-columns:auto 1fr;gap:24px;align-items:start;margin-top:16px' }, [HT.el('div', { html: cardSvg(c, rev) }), HT.el('div', {}, [HT.el('div', { class: 'kpi' }, [HT.el('div', { class: 'lbl' }, `"${question}"`), HT.el('div', { class: 'val' }, `${c[1]} ${rev ? '(역방향)' : ''}`), HT.el('div', { class: 'sub' }, `${c[2]} · 핵심어: ${rev ? c[4] : c[3]}`)]), HT.rows([['사랑·관계', c[5]], ['일·직장', c[6]], ['돈', c[7]], ['조언', c[8]], rev ? ['역방향 해석', `정방향의 뜻(${c[3]})이 막히거나 지나친 상태 — ${c[4]}`, 'sub'] : null]), HT.shareButtons(share, { title: `타로 · "${question}"`, big: `${c[1]} ${rev ? '역방향' : ''}`, lines: [c[2] + ' · ' + (rev ? c[4] : c[3]), '조언: ' + c[8]], file: 'tarot' })])]),
