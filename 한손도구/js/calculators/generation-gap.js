...
HT.register({
  id: 'generation-gap', cat: '내 재무', order: 3, original: false, name: '부모 세대 vs 나', keywords: '부모 세대 비교 PIR 집값 연봉 배수 물가 최저임금 세대 격차',
  ...
  render(root) {
    ...
    function calc(v) {
      ...
      const share = `부모님이 내 나이(${v.age}세)였던 ${then}년: 서울 아파트 ${HT.wonKor(aptThen)} = 연봉 ${HT.fmt(pirThen, 1)}년치. 지금 나: ${HT.wonKor(v.house)} = 연봉 ${HT.fmt(pirNow, 1)}년치. 집값 ${HT.fmt(aptNow / aptThen, 1)}배, 임금 ${HT.fmt(wageNow / wageThen, 1)}배, 물가 ${HT.fmt(cpiRatio, 1)}배 — 한손도구 부모 세대 vs 나`;
