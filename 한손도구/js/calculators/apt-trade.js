HT.register({
  ...
  note: '국토교통부 API는 브라우저 직접 호출(CORS)을 허용하지 않습니다. <code>tools/proxy.js</code>를 Node로 실행한 뒤(기본 http://localhost:8787) 인증키를 넣어 조회하세요. 해제된 거래(해제 여부 O)는 완료되지 않은 거래이므로 시세 판단에서 빼는 것이 좋습니다.',
  render(root) {
