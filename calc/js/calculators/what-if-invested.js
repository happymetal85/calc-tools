...
HT.register({
  id: 'what-if-invested', cat: '금융·투자', order: 0.7, name: '그때 샀더라면', keywords: '그때 샀더라면 과거 투자 코스피 삼성전자 비트코인 금 서울 아파트 수익 후회 계산기',
  ...
  render(root) {
    ...
    function calc(v) {
      ...
      const invested = best.r.invested; const share = `${v.year}년 연말에 ${v.mode === 'lump' ? HT.wonKor(v.lump) + '을 한 번에' : '매년 ' + HT.wonKor(v.annual) + '씩'} ${best.name}에 넣었다면 2025년 말 ${HT.wonKor(best.r.value)} (${HT.fmt(best.r.mult, 1)}배). 예금이면 ${HT.wonKor(dep)} — 숫자맛집 그때 샀더라면`;
